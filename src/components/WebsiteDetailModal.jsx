...
export const WebsiteDetailModal = () => {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/80 backdrop-blur-md animate-fadeIn">
      <div className="relative w-full max-w-2xl max-h-[90vh] overflow-y-auto glass-panel rounded-3xl border border-slate-700/80 shadow-2xl p-6 sm:p-8 text-left">
        ...
        <div className="flex items-center space-x-4 mb-6">
          ...
          <div>
            <div className="flex items-center space-x-3">
              <h2 className="text-2xl font-bold font-heading text-white">{site.name}</h2>
              <span className={`text-xs font-extrabold px-2.5 py-0.5 rounded-full border ${getRiskBadge(site.riskLevel)}`}>
                {site.riskLevel} Risk
              </span>
            </div>
            <p className="text-xs text-slate-400 font-mono mt-0.5">{site.domain} • {site.category}</p>
          </div>
        </div>
        ...
        <div className="mb-6 p-4 rounded-2xl bg-slate-900/60 border border-slate-800">
          ...
          <div className="grid grid-cols-2 sm:grid-cols-3 gap-2">
            {site.dataItems.map((item, i) => (
              <div key={i} className="flex items-center space-x-2 p-2 rounded-xl bg-slate-800/80 border border-slate-700/50 text-xs font-medium text-slate-200">
          ...
          <div className="space-y-2.5">
            {site.consents.map((consent) => {
              ...
              return (
                <div
                  key={consent.id}
                  className="flex items-center justify-between p-3.5 rounded-xl bg-slate-800/40 border border-slate-700/60"
                >
                  <div>
                    <p className="text-xs font-bold text-white">{consent.consentType}</p>
        ...
        <div className="mb-6 p-4 rounded-2xl bg-indigo-950/30 border border-indigo-500/30">
          <div className="flex items-center justify-between mb-2">
            ...
            {!showAiSummary && (
              <button
                ...
                disabled={aiLoading}
                className="text-xs font-semibold px-3 py-1 rounded-lg bg-indigo-600 hover:bg-indigo-500 text-white transition-all flex items-center space-x-1"
              >
                {aiLoading ? (
                  <>
          ...
          {showAiSummary ? (
            <div className="mt-3 text-xs text-indigo-100/90 leading-relaxed font-sans bg-indigo-900/20 p-3 rounded-xl border border-indigo-500/20">
              <p className="font-medium text-slate-200 mb-2">{site.aiSummary}</p>
            ...
            <p className="text-xs text-slate-400">Click summarize to get a 2-sentence plain English breakdown of {site.name}'s legal policy.</p>
          )}
        </div>
